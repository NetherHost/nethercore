...
class TicketStaff {
            ...
            await interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(
                            "Manage the current support ticket here."
                        )
                        .setColor("Red")
                        .addFields(
                            {
        ...
        try {
            ...
            if (!ticketSettings) {
                errorHandler.execute(
                    new Error(
                        `TicketSettings document not found in remote database.`
                    )
                );
                return interaction.reply({
    ...
    public async getResponseTimeStats({
        interaction,
        client,
    }: TicketStaffProps) {
        try {
            ...
            const lastUpdated = ticketSettings.stats.responseTimeLastUpdated
                ? `<t:${Math.floor(
                      ticketSettings.stats.responseTimeLastUpdated.getTime() /
                          1000
                  )}:R>`
                : "Never";
            ...
            await interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(
                            "Current statistics for the ticket system"
                        )
                        .setColor("Red")
                        .addFields(
                            {
