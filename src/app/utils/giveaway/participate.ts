        ...
        try {
            ...
            if (!giveaway) {
                ...
                errorHandler.execute(
                    new Error(`Giveaway ${giveawayId} not found.`)
                );
                return {
                    success: false,
                    error: "That giveaway does not exist.",
                };
            }
            ...
            if (giveaway.ended) {
                ...
                return {
                    success: false,
                    error: "That giveaway has already ended.",
                };
            }
            ...
            try {
                ...
                if (!giveawayMessage) {
                    ...
                    errorHandler.execute(
                        new Error("Unable to fetch giveaway message.")
                    );
                    return {
                        success: false,
                        error: "Unable to fetch giveaway message.",
                    };
                }
            } catch (error) {
                console.error("Unable to fetch giveaway message.");
            ...
            return {
                success: true,
                action: giveaway.participants.includes(userId)
                    ? "added"
                    : "removed",
            };
        } catch (error: any) {
