    ...
    private getRandomWinners(
        ...
        for (let i = 0; i < count && participantsCopy.length > 0; i++) {
            const randomIndex = Math.floor(
                Math.random() * participantsCopy.length
            );
    ...
    public async execute(
        ...
        try {
            ...
            const newWinners = this.getRandomWinners(
                ...
                Math.min(
                    count,
                    giveaway.participants.length - giveaway.winners.length
                )
            );
            ...
            if (newWinners.length === 0) {
                errorHandler.execute(
                    new Error(
                        `No eligible participants for reroll: ${giveawayId}`
                    )
                );
                return {
                    success: false,
            ...
            try {
                ...
                errorHandler.execute(
                    new Error(
                        `Error sending reroll notification: ${giveawayId}`
                    )
                );
            }
